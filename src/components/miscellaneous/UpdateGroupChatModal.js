...
const UpdateGroupChatModal = ({ fetchMessages, fetchAgain, setFetchAgain }) => {
  ...
  useEffect(() => {
    ...
    socket.on("group name updated", (updatedGroup) => {
      if (selectedChat && selectedChat._id === updatedGroup._id) {
        console.log("Group name updated:", updatedGroup);
        setSelectedChat(updatedGroup);
        setFetchAgain((prev) => !prev); // Trigger a re-render
      }
    });
  ...
  const handleRename = async () => {
    ...
    try {
      ...
      socket.emit("group name updated", data); // Emit event to update group name
